...
class Hal < Player
  ...
  def choose
    self.move = [Scissors, Scissors, Rock, Lizard, Spock].sample.new
  end
end
...
class Sonny < Player
  ...
  def choose
    self.move = [Rock, Paper, Scissors].sample.new
  end
end

class Number_5 < Player
  def set_name
    self.name = 'Number 5'
  end
...
class RPSGame
  ...
  COMPUTER_PLAYERS = [R2D2, Hal, Chappie, Sonny, Number_5]

  def initialize
    @human = Human.new
    @computer = COMPUTER_PLAYERS.sample.new
    @history = History.new
    @round = 1
  end
